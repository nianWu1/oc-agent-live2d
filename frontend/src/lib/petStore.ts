...
//
// statusUrl empty → local only (mirror primary mini-pet-state).
// statusUrl set   → poll that GET /status (e.g. tunnel :9999) when reachable.
export interface ExtraMascotConfig {
  petId: string
  /** Empty = local/mirror primary. Non-empty = sync from this /status URL. */
  statusUrl?: string
  /** Optional label shown in the picker (e.g. "Windows"). */
  label?: string
}

function normalizeExtraMascot(raw: unknown): ExtraMascotConfig | null {
  if (typeof raw === 'string' && raw) return { petId: raw, statusUrl: '' }
  if (raw && typeof raw === 'object') {
    const o = raw as Record<string, unknown>
    const petId = typeof o.petId === 'string' ? o.petId : (typeof o.id === 'string' ? o.id : '')
    if (!petId) return null
    const statusUrl = typeof o.statusUrl === 'string' ? o.statusUrl : ''
    const label = typeof o.label === 'string' ? o.label : undefined
    return { petId, statusUrl, label }
  }
  return null
}

export async function loadExtraMascots(): Promise<ExtraMascotConfig[]> {
  const store = await getPetStore()
  const v = await store.get('extra_mascots')
  if (!Array.isArray(v)) return []
  return v.map(normalizeExtraMascot).filter((x): x is ExtraMascotConfig => !!x)
}

export async function saveExtraMascots(mascots: ExtraMascotConfig[]): Promise<void> {
  const store = await getPetStore()
  await store.set(
    'extra_mascots',
    mascots.map((m) => ({
