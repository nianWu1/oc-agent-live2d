...
function ExtraMascotControls({
  ...
  const handlePick = useCallback(
    async (petId: string) => {
      ...
      onNativeDialogStart?.()
      const statusUrl = draftStatusUrl.trim()
      try {
        const label = (await invoke('spawn_extra_mascot', {
          petId,
          statusUrl: statusUrl || null,
        })) as string
        setEntries((prev) => {
          const next = [...prev, { label, petId, statusUrl }]
          void persist(next)
          return next
        })
      } catch (e) {
    ...
    [busy, onNativeDialogStart, onNativeDialogEnd, persist, draftStatusUrl],
  )
  ...
  const handleChangeExtra = useCallback(
    async (label: string, petId: string) => {
      ...
      onNativeDialogStart?.()
      try {
        const prev = entries.find((e) => e.label === label)
        const statusUrl = prev?.statusUrl ?? ''
        try { await invoke('close_extra_mascot', { label }) } catch {}
        const newLabel = (await invoke('spawn_extra_mascot', {
          petId,
          statusUrl: statusUrl || null,
        })) as string
        setEntries((prevEntries) => {
          const next = prevEntries.map((e) =>
            e.label === label ? { label: newLabel, petId, statusUrl } : e,
          )
          void persist(next)
          return next
        })
    ...
    [busy, onNativeDialogStart, onNativeDialogEnd, persist, entries],
  )

  const handleUpdateStatusUrl = useCallback(
    async (label: string, statusUrl: string) => {
      ...
      try {
        ...
        setEntries((prev) => {
          const next = prev.map((e) =>
            e.label === label ? { ...e, label: newLabel, statusUrl: statusUrl.trim() } : e,
          )
          void persist(next)
          return next
        })
      } catch (e) {
        console.warn('[extra-mascot] statusUrl update failed:', e)
      } finally {
        setBusy(false)
        setTimeout(() => onNativeDialogEnd?.(), 600)
      }
    },
    [entries, busy, onNativeDialogStart, onNativeDialogEnd, persist],
  )
    ...
    <div className={bare ? 'p-3 space-y-2' : 'border-t border-white/5 p-3 space-y-2'}>
          ...
          return (
            ...
            >
              ...
              <input
                ...
                className="w-[42%] max-w-[220px] shrink-0 text-[11px] px-2 py-1 rounded-md bg-black/40 border border-white/10 text-white/70 placeholder:text-white/25 outline-none focus:border-white/30"
              />
              <button
                data-no-drag
        ...
        <div className="px-1 pb-1">
          ...
          <input
            data-no-drag
            type="text"
            value={draftStatusUrl}
            onChange={(e) => setDraftStatusUrl(e.target.value)}
            placeholder="http://127.0.0.1:9999/status"
            className="w-full text-[12px] px-2.5 py-1.5 rounded-lg bg-black/40 border border-white/10 text-white/80 placeholder:text-white/25 outline-none focus:border-white/30"
          />
        </div>
        <button
          data-no-drag
