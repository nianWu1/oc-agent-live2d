...
/// Live registry of extra (multi-pet) mascots: (label, petId, statusUrl).
static EXTRA_MASCOTS: std::sync::Mutex<Vec<(String, String, String)>> = std::sync::Mutex::new(Vec::new());

/// Spawn an extra mascot window. Optional `status_url` binds this pet to an
/// independent Cursor progress bridge (`GET /status`); empty means mirror the
/// primary mini's aggregated `mini-pet-state`.
#[tauri::command]
async fn spawn_extra_mascot(
    app: tauri::AppHandle,
    pet_id: String,
    ...
    let status = status_url.unwrap_or_default();
    let encoded_status = urlencoding_encode(&status);
    let url = format!(
        "index.html#/mini?extra=1&pet={}&statusUrl={}",
        pet_id, encoded_status
    );
    let result = spawn_mascot_window(app, label.clone(), url, n, false);
    if result.is_ok() {
        if let Ok(mut reg) = EXTRA_MASCOTS.lock() {
            reg.push((label, pet_id, status));
        }
    }
    result
}

/// Minimal URL-encode for query values (status URLs).
...
/// List the currently-open extra mascots as `[{ label, petId, statusUrl }]`.
#[tauri::command]
